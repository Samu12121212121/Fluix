...
export const generarThumbnailCatalogo = onObjectFinalized(
  ...
  async (event) => {
    ...
    try {
      ...
      console.log(
        `✅ Thumbnail generado para ${productoId} en empresa ${empresaId}`
      );
    } catch (err) {
...
export const scheduledAlertaPreciosAntiguos = onSchedule(
  {
  ...
  async () => {
    ...
    const haceUnAnio = admin.firestore.Timestamp.fromDate(
    ...
    for (const empresaDoc of empresasSnap.docs) {
      ...
      const productosAntiguos: string[] = [];

      for (const doc of productosSnap.docs) {
        const d = doc.data();
        const fechaUltimoCambio =
          d.fecha_ultimo_cambio_precio ?? d.fecha_creacion;
        if (!fechaUltimoCambio) continue;

        const ts = fechaUltimoCambio as admin.firestore.Timestamp;
        if (ts.toMillis() < haceUnAnio.toMillis()) {
          productosAntiguos.push(d.nombre as string);
        }
      }

      if (productosAntiguos.length === 0) continue;
      ...
      try {
        ...
        console.log(
          `✅ Alerta enviada a empresa ${empresaId}: ${n} productos sin actualizar`
        );
      } catch (err) {
