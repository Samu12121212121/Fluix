 ...
 *     46 = resultado (27 - 45)
 */
export function calculate303(transactions: any[], period: string): any {
  const r: any = {
    period,
    model_code: '303',
    casillas: {
    ...
    tx_processed: [] as string[],
    tx_skipped: [] as { id: string; reason: string }[],
    warnings: [] as string[],
  };
...
function processTx303(tx: any, r: any): void {
  ...
  if (scheme === 'margin_scheme') throw new Error('Régimen margen: excluido de 303');
  ...
  if (tx.type === 'invoice_sent' || tx.type === 'invoice_issued') {
    if (scheme === 'standard') {
      if (rate === 4)       { r.casillas['01'] += base; r.casillas['02'] += vat; }
      else if (rate === 10) { r.casillas['04'] += base; r.casillas['05'] += vat; }
      else if (rate === 21) { r.casillas['07'] += base; r.casillas['08'] += vat; }
    }
  }
  ...
  if (tx.type === 'invoice_received') {
    if (scheme === 'standard') {
      ...
      r.casillas['10'] += base; r.casillas['11'] += calcVat;
      r.casillas['34'] += base; r.casillas['35'] += calcVat;
    } else if (scheme === 'reverse_charge_domestic') {
      const calcVat = base * (rate / 100 || 0.21);
      r.casillas['12'] += base; r.casillas['13'] += calcVat;
      r.casillas['28'] += base; r.casillas['29'] += calcVat;
