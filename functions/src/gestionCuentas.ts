...
// ─────────────────────────────────────────────────────────────────────────────
export const PLANES_CONFIG: Record<
  string,
  {
    id: string;
    nombre: string;
 ...
 * }
 */
export const crearCuentaConPlan = onCall(
  { region: REGION },
  async (request) => {
    // 1. Autenticación
    ...
    if (!email || !planId || !nombreEmpresa) {
      throw new HttpsError(
        "invalid-argument",
        "email, planId y nombreEmpresa son obligatorios."
      );
    }
    ...
    if (!plan) {
      throw new HttpsError(
        "invalid-argument",
        `Plan desconocido: ${planId}. Usa: ${Object.keys(PLANES_CONFIG).join(", ")}`
      );
    }
    ...
    try {
      ...
      throw new HttpsError("internal", `Error creando usuario: ${authErr.message}`);
    }
    ...
    try {
      ...
      const configuracionRef = empresaRef
        ...
        .doc("modulos");
      const modulosConfig: Record<string, boolean> = {};
      ...
      await batch.commit();
      console.log(
        `✅ Cuenta creada: uid=${uid}, empresa=${empresaId}, plan=${planId}`
      );
    } catch (err) {
      ...
      throw new HttpsError(
        "internal",
        `Error creando datos en Firestore: ${(err as Error).message}`
      );
    }
 ...
 * }
 */
export const actualizarPlanEmpresa = onCall(
  { region: REGION },
  async (request) => {
    const callerUid = request.auth?.uid;
    if (!callerUid)
    ...
    if (!empresaId || !nuevoPlanId) {
      throw new HttpsError(
        "invalid-argument",
        "empresaId y nuevoPlanId son obligatorios."
      );
    }

    const nuevoPlan = PLANES_CONFIG[nuevoPlanId];
    if (!nuevoPlan) {
      throw new HttpsError("invalid-argument", `Plan desconocido: ${nuevoPlanId}`);
    }
    ...
    if (suscripcionSnap.exists && extenderDias === 0) {
      ...
      fechaFin =
        suscripcionSnap.data()?.fecha_fin?.toDate() ?? new Date();
    } else {
    ...
    const modulosNuevos: Record<string, boolean> = {};
    ...
    console.log(
      `✅ Plan actualizado: empresa=${empresaId}, nuevo plan=${nuevoPlanId}`
    );
 ...
 */
export const listarCuentasClientes = onCall(
  { region: REGION },
  async (request) => {
    const callerUid = request.auth?.uid;
    if (!callerUid)
    ...
    for (const doc of empresasSnap.docs) {
      ...
      result.push({
        ...
        planId: suscripcion?.plan ?? "sin_plan",
        planNombre: suscripcion?.plan_nombre ?? "Sin plan",
        estado: suscripcion?.estado ?? "DESCONOCIDO",
        fechaFin: suscripcion?.fecha_fin?.toDate?.()?.toISOString() ?? null,
        activa: empresa.activa ?? false,
        fechaCreacion: empresa.fecha_creacion?.toDate?.()?.toISOString() ?? null,
        // Campos V2
        packsActivos: suscripcion?.packs_activos ?? [],
        addonsActivos: suscripcion?.addons_activos ?? [],
        empleadosNomina: suscripcion?.empleados_nomina ?? 0,
        precioTotal: suscripcion?.precio_total ?? 0,
      });
    }
 ...
 * }
 */
export const webhookPagoWeb = onRequest(
  ...
  async (req, res) => {
    ...
    const secretEnv = process.env.FLUIX_WEBHOOK_SECRET ?? "";
    const secretHeader = req.headers["x-fluix-secret"] as string | undefined;
        ...
        if (tokens.length > 0) {
          await admin.messaging().sendEachForMulticast({
            ...
            notification: {
              ...
              body: `${nombreEmpresa ?? email} — ${planNombre} — €${importe}`,
            },
            data: {
              tipo: "pago_web",
              email,
      ...
      if (crearCuentaAuto && plan) {
        try {
          ...
          const newUser = await admin.auth().createUser({
            email: email.toLowerCase().trim(),
            password: tempPassword,
            displayName: nombreEmpresa ?? email,
          });
