...
exports.verificarLoginIntento = void 0;
 ...
 */
exports.verificarLoginIntento = (0, https_1.onRequest)({ region: REGION, cors: true }, async (req, res) => {
    if (req.method !== "POST") {
        res.status(405).json({ error: "Method Not Allowed" });
        return;
    }
    const { email, exito } = req.body;
    ...
    try {
        ...
        const result = await db.runTransaction(async (tx) => {
            ...
            if (contador >= MAX_INTENTOS) {
                updateData.bloqueado_hasta = admin.firestore.Timestamp.fromMillis(now + BLOQUEO_MINUTOS * 60 * 1000);
            }
        ...
        _limpiarRegistrosAntiguos().catch((err) => console.warn("Error limpieza login_intentos:", err));
 ...
 */
async function _limpiarRegistrosAntiguos() {
    const cutoff = admin.firestore.Timestamp.fromMillis(Date.now() - LIMPIEZA_HORAS * 60 * 60 * 1000);
    const snap = await db
...
//# sourceMappingURL=fuerzaBruta.js.map