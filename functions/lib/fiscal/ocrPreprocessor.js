...
exports.preprocesarTextoOCR = preprocesarTextoOCR;
...
function preprocesarTextoOCR(rawText) {
    const avisos = [];
    ...
    if (esNotarial) {
        ...
        bloqueAviso.push('⚠️ AVISO PREPROCESADOR: Este documento es de un NOTARIO, REGISTRADOR o PROCURADOR.', 'Es muy probable que contenga importes NO SUJETOS a IVA (timbres, suplidos, tasas, aranceles).', 'BUSCA ACTIVAMENTE non_subject_amount aunque no veas columnas explícitas.', 'Si el total no cuadra con base+IVA-retención, calcula non_subject_amount por diferencia.');
    }
    if (tieneIndiciosNoSujeto) {
        avisos.push('indicios_no_sujeto_detectados');
        bloqueAviso.push('⚠️ AVISO PREPROCESADOR: Se han detectado términos que indican partes NO SUJETAS a IVA.', 'Extrae el importe no sujeto en el campo non_subject_amount.');
    }
    if (ocrAplanado && esNotarial) {
        avisos.push('ocr_columnas_aplanadas');
        bloqueAviso.push('⚠️ AVISO PREPROCESADOR: El OCR puede haber aplanado columnas.', `Se detectaron ${lineasConMultiplesImportes.length} línea(s) con múltiples importes:`, ...lineasConMultiplesImportes.map(l => `  → "${l.trim()}"`), 'Usa la fórmula de cálculo por diferencia si es necesario:');
    }
...
//# sourceMappingURL=ocrPreprocessor.js.map