...
const invoiceExtractionV1_1 = require("./prompts/invoiceExtractionV1");
if (!admin.apps.length)
    admin.initializeApp();
const SYSTEM_PROMPT = invoiceExtractionV1_1.SYSTEM_PROMPT_INVOICE_ES;
// ═══════════════════════════════════════════════════════════════
...
Devuelve solo el JSON según el schema del bloque 8.`;
    ...
    return { data, promptVersion: invoiceExtractionV1_1.PROMPT_VERSION };
}
...
exports.processInvoice = (0, https_1.onCall)({
    ...
    try {
        ...
        const userDoc = await admin
            ...
            .doc(uid)
            .get();
        console.log(`userDoc exists: ${userDoc.exists}, empresa_id en doc: ${(_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.empresa_id}`);
        if (!userDoc.exists || ((_b = userDoc.data()) === null || _b === void 0 ? void 0 : _b.empresa_id) !== empresaId) {
        ...
        const empresaDoc = await admin
            ...
            .doc(empresaId)
            .get();
        const activePacks = ((_c = empresaDoc.data()) === null || _c === void 0 ? void 0 : _c.active_packs) || [];
        ...
        const extractionRef = await admin
            ...
            prompt_version: invoiceExtractionV1_1.PROMPT_VERSION,
            raw_text: rawText,
            status: "processing",
        ...
        if (invoiceData.supplier_tax_id && invoiceData.invoice_number) {
            ...
            if (!dupQuery.empty) {
                ...
                for (const existing of dupQuery.docs) {
                    ...
                    const existingDate = existingData.invoice_date.toDate();
                    const daysDiff = Math.abs((invoiceDate.getTime() - existingDate.getTime()) / (1000 * 60 * 60 * 24));
                    if (daysDiff <= 1) {
                        duplicadoConfirmado = true;
                        validation.errors.push(`duplicate_confirmed: factura idéntica ya registrada (${existing.id}) el ${existingDate.toISOString().split("T")[0]}`);
                        console.log(`DUPLICADO CONFIRMADO: tx existente ${existing.id}`);
                        break;
                    }
                    if (daysDiff <= 7) {
                        validation.warnings.push(`duplicate_likely: mismo NIF y número pero fecha distinta (diferencia ${Math.round(daysDiff)} días, tx ${existing.id})`);
                    }
                }
        ...
        const txRef = await admin
            ...
            _ai_prompt_version: invoiceExtractionV1_1.PROMPT_VERSION,
        ...
        if (tipoDocumento === "ingreso") {
            // BUG #7 FIX: Ingresos van a facturas/ (emitidas), NO a facturas_recibidas/
            console.log("Guardando en facturas/ (emitida)...");
            ...
            await facturaEmitidaRef.set({
                ...
                cliente_nombre: invoiceData.supplier_name || "",
                datos_fiscales: {
                    nif: invoiceData.supplier_tax_id || null,
                    razon_social: invoiceData.supplier_legal_name || invoiceData.supplier_name || null,
                    direccion: invoiceData.supplier_address || null,
                    pais: invoiceData.supplier_country || "ES",
                },
                subtotal: baseAmount,
                total_iva: vatAmount,
                total: totalAmount,
                ...
                estado: status === "posted" ? "pendiente" : "pendiente",
                tipo_factura: "venta_directa",
                notas: `Procesada por IA (${ocrEngine})`,
            ...
            await extractionRef.update({
                ...
                factura_recibida_id: null,
                factura_emitida_id: facturaEmitidaRef.id,
            });
            ...
            // GASTOS: guardar en facturas_recibidas/ (comportamiento original)
            console.log("Guardando factura_recibida...");
            const baseAmount2 = parseFloat(invoiceData.base_amount || "0");
            const vatRate2 = parseFloat(invoiceData.vat_rate || "0");
            ...
            await facturaRecibidaRef.set({
                ...
                base_imponible: baseAmount2,
                porcentaje_iva: vatRate2,
                importe_iva: vatAmount2,
                iva_deducible: !(invoiceData.tax_tags || []).includes("VAT_NOT_DEDUCTIBLE"),
                descuento_global: 0,
                recargo_equivalencia: recargoRate,
                total_con_impuestos: totalAmount2,
        ...
        console.error("Tipo:", (_d = error === null || error === void 0 ? void 0 : error.constructor) === null || _d === void 0 ? void 0 : _d.name);
        console.error("Mensaje:", error === null || error === void 0 ? void 0 : error.message);
        console.error("Código:", error === null || error === void 0 ? void 0 : error.code);
        console.error("Stack:", error === null || error === void 0 ? void 0 : error.stack);
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError("internal", (error === null || error === void 0 ? void 0 : error.message) || "Error interno desconocido");
    }
});
// ═══════════════════════════════════════════════════════════════
// HELPERS
...
function validateInvoice(data) {
    ...
    if (data.invoice_date) {
        ...
        if (invDate < fourYearsAgo)
            warnings.push("Fecha muy antigua (>4 años)");
    }
...
function decideStatus(data, validation) {
    ...
    if (!data.invoice_number)
        return "needs_review";
    if (validation.warnings.some((w) => w.toLowerCase().includes("duplicado") || w.includes("duplicate")))
        return "needs_review";
    return "posted";
}
