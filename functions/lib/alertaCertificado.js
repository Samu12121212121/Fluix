...
exports.scheduledAlertaCertificado = void 0;
...
async function enviarNotificacion(empresaId, titulo, cuerpo, data = {}) {
    ...
    const mensaje = {
        ...
        data: Object.assign({ empresa_id: empresaId, tipo: "certificado_expiracion" }, data),
        android: {
            priority: "high",
            notification: {
    ...
    try {
        ...
        console.log(`[AlertaCert] Empresa ${empresaId}: ${respuesta.successCount}/${tokens.length} notificaciones enviadas`);
...
// ═══════════════════════════════════════════════════════════════════════════════
exports.scheduledAlertaCertificado = (0, scheduler_1.onSchedule)({
    ...
    for (const empresaDoc of empresasSnap.docs) {
        ...
        try {
            ...
            if (!validoHasta)
                continue;
            const fechaExpiracion = validoHasta.toDate();
            fechaExpiracion.setHours(0, 0, 0, 0);
            const diasRestantes = Math.floor((fechaExpiracion.getTime() - hoy.getTime()) / (1000 * 60 * 60 * 24));
            const titular = certData.titular || "certificado digital";
            const nif = certData.nif || "";
            console.log(`[AlertaCert] Empresa ${empresaId}${nif ? " (" + nif + ")" : ""}: ${diasRestantes} días restantes`);
            // Determinar si hay que enviar alerta y con qué nivel
            if (diasRestantes < 0) {
                // Expirado — alerta diaria urgente
                await enviarNotificacion(empresaId, "🔴 Certificado digital EXPIRADO", `Tu certificado digital expiró hace ${Math.abs(diasRestantes)} días. ` +
                    "Los modelos fiscales no se podrán firmar digitalmente.", { dias_restantes: diasRestantes.toString(), tipo_alerta: "expirado" });
                alertasEnviadas++;
                // Registrar en Firestore
                await _registrarAlerta(empresaId, "expirado", diasRestantes);
            }
            else if (diasRestantes === 0) {
                await enviarNotificacion(empresaId, "🔴 Tu certificado digital expira HOY", "Renueva el certificado digital de inmediato para mantener " +
                    "la firma electrónica de los modelos fiscales.", { dias_restantes: "0", tipo_alerta: "hoy" });
                alertasEnviadas++;
                await _registrarAlerta(empresaId, "hoy", 0);
            }
            else if (diasRestantes <= 7) {
                await enviarNotificacion(empresaId, `🔴 Certificado digital: ${diasRestantes} días`, `El certificado ${titular.substring(0, 30)} expira en ${diasRestantes} día(s). ` +
                ...
                await enviarNotificacion(empresaId, "⚠️ Certificado digital: 30 días", `El certificado digital expira el ${_fmtFecha(fechaExpiracion)}. ` +
                ...
                await enviarNotificacion(empresaId, "⚠️ Certificado digital: 60 días", `El certificado digital expira el ${_fmtFecha(fechaExpiracion)}. ` +
    ...
    console.log(`[AlertaCert] Completado: ${alertasEnviadas} alertas enviadas, ${errores} errores`);
});
async function _registrarAlerta(empresaId, tipoAlerta, diasRestantes) {
    try {
        await db
            .collection("empresas")
            .doc(empresaId)
...
//# sourceMappingURL=alertaCertificado.js.map