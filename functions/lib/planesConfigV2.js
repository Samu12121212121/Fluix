...
function calcularModulosActivos(packsActivos, addonsActivos) {
    const modulos = new Set(exports.PLAN_BASE.modulosIncluidos);
    for (const packId of packsActivos) {
        const pack = exports.PACKS[packId];
        if (pack) {
            pack.modulosAdicionales.forEach((m) => modulos.add(m));
        }
    }
    for (const addonId of addonsActivos) {
        const addon = exports.ADDONS[addonId];
        if (addon) {
 ...
 */
exports.actualizarModulosSegunPlan = (0, https_1.onCall)({ region: REGION }, async (request) => {
    var _a;
    ...
    if (!empresaId) {
        throw new https_1.HttpsError("invalid-argument", "empresaId es obligatorio.");
    }
    // 1. Leer suscripción
    ...
    if (!suscripcionSnap.exists) {
        throw new https_1.HttpsError("not-found", "No existe suscripción para esta empresa.");
    }
    const suscripcionData = suscripcionSnap.data();
    const packsActivos = suscripcionData.packs_activos || [];
    const addonsActivos = suscripcionData.addons_activos || [];
    // 2. Calcular módulos
    ...
    await db
        ...
        .doc("modulos")
        .set(Object.assign(Object.assign({}, modulosMap), { modulos: modulosActivos.map((m) => ({ id: m, activo: true })), fecha_actualizacion: admin.firestore.FieldValue.serverTimestamp(), generado_por: "actualizarModulosSegunPlan" }), { merge: false });
    console.log(`✅ Módulos actualizados para empresa=${empresaId}: ${modulosActivos.join(", ")}`);
    return {
        ok: true,
        empresaId,
        modulosActivos,
 ...
 */
exports.migracionPlanesV2 = (0, https_1.onCall)({ region: REGION, timeoutSeconds: 300 }, async (request) => {
    var _a;
    const callerUid = (_a = request.auth) === null || _a === void 0 ? void 0 : _a.uid;
    if (!callerUid) {
        throw new https_1.HttpsError("unauthenticated", "Debes estar autenticado.");
    ...
    for (const empresaDoc of empresasSnap.docs) {
        ...
        try {
            ...
            if (suscData.modulos_activos && Array.isArray(suscData.modulos_activos)) {
                ...
                modulosActuales = Object.keys(configData).filter((k) => k !== "fecha_actualizacion" && k !== "generado_por" && k !== "modulos");
            }
            // 5. Inferir packs
            const packs = [];
            if (modulosActuales.includes("facturacion") ||
            ...
            // 6. Inferir addons
            const addons = [];
            if (modulosActuales.includes("whatsapp"))
                addons.push("whatsapp");
            if (modulosActuales.includes("tareas"))
                addons.push("tareas");
            if (modulosActuales.includes("nominas"))
                addons.push("nominas");
            // 7. Calcular precio
            const precioTotal = calcularPrecioTotal(packs, addons);
            const planAnterior = suscData.plan || "basico";
            resultados.push({
                empresaId,
                nombre,
                planAnterior,
            ...
            if (!dryRun) {
                ...
                console.log(`✅ ${empresaId} (${nombre}): migrada — packs=[${packs}] addons=[${addons}] precio=${precioTotal}€`);
            }
            else {
                console.log(`🔍 [DRY] ${empresaId} (${nombre}): packs=[${packs}] addons=[${addons}] precio=${precioTotal}€`);
            }
        }
        catch (err) {
 ...
 * }
 */
exports.actualizarPlanEmpresaV2 = (0, https_1.onCall)({ region: REGION }, async (request) => {
    var _a, _b, _c, _d;
    const callerUid = (_a = request.auth) === null || _a === void 0 ? void 0 : _a.uid;
    if (!callerUid) {
        throw new https_1.HttpsError("unauthenticated", "Debes estar autenticado.");
    ...
    await verificarPropietarioPlatforma(callerUid);
    const { empresaId, packsActivos = [], addonsActivos = [], empleadosNomina = 0, extenderDias = 365, } = request.data;
    if (!empresaId) {
        throw new https_1.HttpsError("invalid-argument", "empresaId es obligatorio.");
    }
    // Validar packs
    for (const p of packsActivos) {
        if (!exports.PACKS[p]) {
            throw new https_1.HttpsError("invalid-argument", `Pack desconocido: ${p}`);
        }
    }
    // Validar addons
    for (const a of addonsActivos) {
        if (!exports.ADDONS[a]) {
            throw new https_1.HttpsError("invalid-argument", `Add-on desconocido: ${a}`);
        }
    }
    // Calcular
    const modulosActivos = calcularModulosActivos(packsActivos, addonsActivos);
    ...
    if (suscripcionSnap.exists && extenderDias === 0) {
        fechaFin = (_d = (_c = (_b = suscripcionSnap.data()) === null || _b === void 0 ? void 0 : _b.fecha_fin) === null || _c === void 0 ? void 0 : _c.toDate()) !== null && _d !== void 0 ? _d : new Date();
    }
    else {
        fechaFin = new Date();
    ...
    batch.set(db.collection("empresas").doc(empresaId).collection("configuracion").doc("modulos"), Object.assign(Object.assign({}, modulosMap), { modulos: modulosActivos.map((m) => ({ id: m, activo: true })), fecha_actualizacion: admin.firestore.FieldValue.serverTimestamp(), generado_por: "actualizarPlanEmpresaV2" }), { merge: false });
    await batch.commit();
    console.log(`✅ Plan V2 actualizado: empresa=${empresaId}, packs=[${packsActivos}], ` +
...
//# sourceMappingURL=planesConfigV2.js.map