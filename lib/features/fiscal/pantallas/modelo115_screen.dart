...
class Modelo115Screen extends StatefulWidget {
  ...
  @override
  State<Modelo115Screen> createState() => _Modelo115ScreenState();
}

class _Modelo115ScreenState extends State<Modelo115Screen> {
  final _svc = Mod115Calculator();
  late int _anio;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('✅ Mod.115 $trimestre $_anio — '
              'Resultado: ${modelo.c05.toStringAsFixed(2)} € '
              '(${modelo.c01} arrendadores)'),
  ...
  Future<void> _generarPdf(Modelo115 m) async {
    if (_empresaConfig == null) return;
    try {
      final bytes = await Mod115Exporter.generarPDF(
        modelo: m, empresa: _empresaConfig!,
      );
      final dir = await getTemporaryDirectory();
      final nombre = 'Mod115_${m.ejercicio}_${m.trimestre}.pdf';
      final archivo = File('${dir.path}/$nombre');
      await archivo.writeAsBytes(bytes);
      await Share.shareXFiles([XFile(archivo.path)],
          text: 'Modelo 115 — ${m.ejercicio} ${m.trimestre}');
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  Future<void> _generarTxt(Modelo115 m) async {
    if (_empresaConfig == null) return;
    try {
      final bytes = Mod115Exporter.generarFicheroTXT(
        modelo: m, empresa: _empresaConfig!,
      );
      final dir = await getTemporaryDirectory();
      final nombre = '115_${m.ejercicio}_${m.trimestre}.txt';
      final archivo = File('${dir.path}/$nombre');
      await archivo.writeAsBytes(bytes);
      await Share.shareXFiles([XFile(archivo.path)],
          text: 'Modelo 115 — ${m.ejercicio} ${m.trimestre} — Fichero AEAT');
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  Widget _buildResumenAnual(List<Modelo115> modelos) {
    final totalRetenciones = modelos.fold(0.0, (s, m) => s + m.c03);
    final presentados = modelos.where((m) => m.estado == EstadoModelo115.presentado).length;
  ...
  Widget _buildTarjetaTrimestre(String trimestre, List<Modelo115> modelos) {
    final modelo = modelos.where((m) => m.trimestre == trimestre).firstOrNull;
    final plazo = Modelo115.calcularPlazoLimite(_anio, trimestre);
    final diasRestantes = plazo.difference(DateTime.now()).inDays;
    final rango = Modelo115.rangoMeses(trimestre);
    ...
    } else if (modelo.estado == EstadoModelo115.presentado) {
      colorEstado = Colors.green;
      estadoTexto = 'Presentado';
    } else {
      colorEstado = Colors.orange;
      estadoTexto = modelo.estado.etiqueta;
    }
                ...
                child: Row(
                  children: [
                    ...
                    _botonAccion('AEAT .txt', Icons.file_download, Colors.teal,
                        () => _generarTxt(modelo)),
                    if (modelo.estado != EstadoModelo115.presentado) ...[
                      const SizedBox(width: 8),
  ...
  Widget _datosModelo(Modelo115 m) {
    if (m.c01 == 0) {
      return Container(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              ...
              Expanded(
                child: Text(
                  'No se han encontrado facturas de arrendamiento en este trimestre.',
                  style: TextStyle(
            ...
            Text(
              'Para incluirlas, marca la opción "Es arrendamiento" al registrar '
              'la factura recibida del local o inmueble.',
    ...
    return Column(
      children: [
        _filaDato('Nº arrendadores', '${m.c01}', '[01]'),
        _filaDato('Base retenciones', '${m.c02.toStringAsFixed(2)} €', '[02]'),
        _filaDato('Retenciones (19%)', '${m.c03.toStringAsFixed(2)} €', '[03]', bold: true),
        if (m.esComplementaria)
          _filaDato('A deducir anterior', '${m.c04.toStringAsFixed(2)} €', '[04]'),
        const Divider(height: 8),
        _filaDato('Resultado', '${m.c05.toStringAsFixed(2)} €', '[05]',
            bold: true,
            color: m.c05 > 0 ? Colors.green.shade700 : Colors.orange.shade700),
        if (m.arrendadores.isNotEmpty) ...[
          const Divider(height: 8),
          ...m.arrendadores.map((a) => Padding(
            padding: const EdgeInsets.symmetric(vertical: 1),
            child: Row(
              children: [
                Text(a.nif, style: TextStyle(fontSize: 10, color: Colors.grey.shade600)),
                const SizedBox(width: 6),
                Expanded(child: Text(a.nombre, style: const TextStyle(fontSize: 11))),
                Text('${a.baseImponible.toStringAsFixed(2)} €',
